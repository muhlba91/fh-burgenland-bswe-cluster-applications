...
CLUSTER_ACCESS_POD=$(kubectl -n fh-bswe-cluster-access get pod -l "app.kubernetes.io/name=cluster-access" -o jsonpath="{.items[0].metadata.name}")
for cluster in ${CLUSTER_IDS}; do
    echo "copying configuration for cluster-${cluster}..."
    kubectl -n fh-bswe-cluster-access cp vclusters/configs/cluster-${cluster}.zip $CLUSTER_ACCESS_POD:/usr/share/nginx/html/files/
done
